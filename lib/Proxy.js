define([
	...
	Proxy.prototype = {
      ...
      process.emitWarning(`Request received at URL: ${request.method} ${request.url}`);
            ...
            process.emitWarning(`POST request data received at runner: ${JSON.stringify(messages)}`);
            ...
            process.emitWarning(`Status for should Wait: ${shouldWait}`);
      ...
      process.emitWarning(`Response code from runner: ${response.statusCode}`)
		},

		_handleFile: function (request, response, instrument, omitContent) {
      process.emitWarning('fileHandling is called');
			function send(contentType, data) {
        process.emitWarning(`File data sent in response: ${data}`);
            ...
            process.emitWarning(`Write stream from path: ${wholePath}`);
      ...
      process.emitWarning(`Response for resolvingSuite: ${resolvedSuites}`);
			response.writeHead(200, {
      ...
      process.emitWarning('🚨 404 is being sent');
        ...
        process.emitWarning('Server is created');
				this._sessions = {};
				this._codeCache = {};
				...
				aspect.after(server, 'close', function () {
					var socket;
          process.emitWarning('Sockets are being destroyed');
          ...
          process.emitWarning('new Socket is connected');
					sockets.push(socket);
            ...
            process.emitWarning('Socket is being closed');
