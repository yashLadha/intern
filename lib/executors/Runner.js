define([
	...
	Runner.prototype = lang.mixin(Object.create(_super), /** @lends module:intern/lib/executors/Runner# */ {
        ...
        process.emitWarning('stopProxy is called');
			...
			function stopTunnel() {
        process.emitWarning('Stop tunnel is called');
				if (self.tunnel && self.tunnel.isRunning) {
					return self.tunnel.stop().then(function () {
            process.emitWarning('Tunnel is closed');
		...
		_beforeRun: function () {
			...
			function startTunnel() {
				...
				return tunnel.start().then(async function () {
          process.emitWarning('Tunnel is created');
