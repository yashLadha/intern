...
BrowserStackTunnel.prototype = util.mixin(Object.create(_super), /** @lends module:digdug/BrowserStackTunnel# */ {
    ...
    process.emitWarning('Sending JOB state');
    process.emitWarning(`Payloading being emitted ${payload}`);
	...
	_start: function () {
		...
		var handle = util.on(childProcess.stdout, 'data', function (data) {
			...
			if (error) {
				...
				handle.remove();
				dfd.resolve();
        process.emitWarning('Completely connected | Binary');
			}
			else {
          ...
          process.emitWarning('Connecting to local binary');
    ...
    // Adding an explicit timeout for delaying the killing of binary.
    // This is to prevent the race condition that is happending.
    setTimeout(function() {
